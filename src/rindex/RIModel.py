...
class RIModel:
	...
	def writeModelToFile(self, filename = "dump.model"):
		print(filename)
	...
	def addUnit(self, unit="", context=[]):
		...
		for entry in context:
			if entry not in self.index_memory.keys():
				...
				self.index_memory[entry] = self.iv.createIndexVectorFromContext([entry])
			self.ContextVectors[unit] += self.index_memory[entry]
